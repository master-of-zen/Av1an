...
fn validate_zones_encoder_changed_no_reset() {
    ...
    assert_eq!(
        ...
        "Zone includes encoder change but previous args were kept. You \
         probably meant to specify \"reset\"."
    );
}
...
fn validate_zones_format_mismatch() {
    ...
    assert_eq!(
        result.err().unwrap().to_string(),
        "Zone specifies using x264, but this cannot be used in the same file \
         as aom"
    );
}
...
fn validate_zones_no_args_reset() {
    ...
    // This is weird, but can technically work for some encoders so we'll allow
    // it.
    let result = Scene::parse_from_zone(input, &args).unwrap();
