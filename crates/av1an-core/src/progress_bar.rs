...
const INDICATIF_PROGRESS_TEMPLATE: &str = if cfg!(windows) {
    ...
    "{elapsed_precise:.bold} ▐{wide_bar:.blue/white.dim}▌ {percent:.bold} \
     {pos} ({fps:.bold}, eta {fixed_eta}{msg})"
} else {
    "{spinner:.green.bold} {elapsed_precise:.bold} \
     ▕{wide_bar:.blue/white.dim}▏ {percent:.bold}  {pos} ({fps:.bold}, eta \
     {fixed_eta}{msg})"
};
...
const INDICATIF_SPINNER_TEMPLATE: &str = if cfg!(windows) {
    // Do not use a spinner on Windows since the default console cannot display
    // the characters used for the spinner
    "{elapsed_precise:.bold} [{wide_bar:.blue/white.dim}]  {pos} frames \
     ({fps:.bold})"
} else {
    "{spinner:.green.bold} {elapsed_precise:.bold} \
     [{wide_bar:.blue/white.dim}]  {pos} frames ({fps:.bold})"
};
    ...
    ProgressStyle::default_bar()
        ...
        .with_key(
            ...
            move |state: &ProgressState, w: &mut dyn Write| {
                let resume_pos = state.pos() - resume_frames;
                if resume_pos == 0
                    || state.elapsed().as_secs_f32() < f32::EPSILON
                {
                    ...
                    write!(
                        w,
                        "{:#}",
                        HumanDuration(Duration::from_secs_f32(
                            spf * remaining as f32
                        ))
                    )
                    .unwrap();
                }
...
pub fn init_progress_bar(len: u64, resume_frames: u64) {
    let pb = if len > 0 {
        PROGRESS_BAR.get_or_init(|| {
            ProgressBar::new(len)
                .with_style(pretty_progress_style(resume_frames))
        })
    } else {
        // Avoid showing `xxx/0` if we don't know the length yet.
        // Affects scenechange progress.
        PROGRESS_BAR.get_or_init(|| {
            ProgressBar::new(len).with_style(spinner_style(resume_frames))
        })
    };
...
static MULTI_PROGRESS_BAR: OnceCell<(MultiProgress, Vec<ProgressBar>)> =
    OnceCell::new();
