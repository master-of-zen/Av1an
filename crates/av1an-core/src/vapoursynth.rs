...
static VAPOURSYNTH_PLUGINS: Lazy<HashSet<String>> = Lazy::new(|| {
    let environment = Environment::new()
        .expect("Failed to initialize VapourSynth environment");
...
pub fn is_dgdecnv_installed() -> bool {
    static DGDECNV_PRESENT: Lazy<bool> = Lazy::new(|| {
        VAPOURSYNTH_PLUGINS.contains("com.vapoursynth.dgdecodenv")
    });
...
pub fn is_bestsource_installed() -> bool {
    static BESTSOURCE_PRESENT: Lazy<bool> = Lazy::new(|| {
        VAPOURSYNTH_PLUGINS.contains("com.vapoursynth.bestsource")
    });
...
fn get_frame_rate(env: &Environment) -> anyhow::Result<f64> {
    ...
    match info.framerate {
        ...
        Property::Constant(fps) => {
            Ok(fps.numerator as f64 / fps.denominator as f64)
        },
    }
}
...
fn get_transfer(env: &Environment) -> anyhow::Result<u8> {
    ...
    let transfer = frame
        ...
        .map_err(|_| {
            anyhow::anyhow!(
                "Failed to get transfer characteristics from VS script"
            )
        })? as u8;
...
pub fn create_vs_file(
    ...
    if chunk_method == ChunkMethod::DGDECNV {
        ...
        load_script.write_all(
            format!(
                "from vapoursynth import \
                 ...
                 DGSource(source={dgindex_path:?}).set_output()"
            )
            .as_bytes(),
        )?;
        ...
        load_script.write_all(
            format!(
                "from vapoursynth import \
                 core\ncore.max_cache_size=1024\ncore.bs.VideoSource({source:?\
                 }, cachepath={cache_file:?}).set_output()"
            )
            .as_bytes(),
        )?;
    } else {
        load_script.write_all(
            // TODO should probably check if the syntax for rust strings and
            // escaping utf and stuff like that is the same as in python
            format!(
                "from vapoursynth import \
                 core\ncore.max_cache_size=1024\ncore.{}({:?}, \
