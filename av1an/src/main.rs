...
pub struct CliOpts {
    ...
    /// by specifying the correct parameter to the encoder. However, the
    /// two should not be used together, and specifying this option will
    /// disable the encoder's internal grain synthesis.
    #[clap(long, help_heading = "Encoding")]
    ...
    /// that are allowed by the encoder, as well as the following av1an
    /// options:
    ///
    /// - `-x`/`--extra-split`
    /// - `--min-scene-len`
