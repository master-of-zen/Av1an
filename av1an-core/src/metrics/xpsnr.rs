...
pub fn validate_libxpsnr() -> anyhow::Result<()> {
  ...
  if !stdr.contains("xpsnr") {
    return Err(anyhow!("FFmpeg is not compiled with XPSNR or is outdated, but target quality or XPSNR plotting was enabled"));
  }
  Ok(())
}
...
pub fn read_xpsnr_file(file: impl AsRef<Path>) -> Vec<f64> {
  ...
  let re = regex::Regex::new(r".*XPSNR y: *([0-9\.]+) *XPSNR u: *([0-9\.]+) *XPSNR v: *([0-9\.]+)")
    .unwrap();
