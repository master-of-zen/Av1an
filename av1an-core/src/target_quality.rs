...
impl TargetQuality {
    fn per_shot_target_quality(
        ...
        let mut score = read_weighted_vmaf(
            self.vmaf_probe(chunk, last_q as usize)?,
            VMAF_PERCENTILE,
        )
        .unwrap();
        ...
        // Branch
        let next_q = if score < self.target { self.min_q } else { self.max_q };

        // Edge case check
        score = read_weighted_vmaf(
            self.vmaf_probe(chunk, next_q as usize)?,
            VMAF_PERCENTILE,
        )
        .unwrap();
            ...
            || (next_q == self.max_q && score > self.target)
        {
            log_probes(
                ...
                next_q,
                score,
                if score < self.target { Skip::Low } else { Skip::High },
            );
            return Ok(next_q);
        }
        ...
        for _ in 0..self.probes - 2 {
            ...
            score = read_weighted_vmaf(
                self.vmaf_probe(chunk, new_point)?,
                VMAF_PERCENTILE,
            )
            .unwrap();
    ...
    fn vmaf_probe(
        ...
        chunk: &Chunk,
        q: usize,
    ) -> Result<PathBuf, Box<EncoderCrash>> {
        ...
        let future = async {
            ...
            let source_pipe_output =
                source_pipe.wait_with_output().await.unwrap();
...
pub fn weighted_search(
    ...
    num1.mul_add(dif1 / tot, num2 * (dif2 / tot))
        .round() as usize
}
...
pub fn interpolate_target_vmaf(
    scores: Vec<(f64, u32)>,
    q: f64,
) -> Result<f64, Error> {
    let mut sorted = scores;
...
pub fn log_probes(
    ...
    debug!("chunk {}: P-Rate={}, {} frames", chunk_idx, probing_rate, frames);
    debug!(
        "chunk {}: TQ-Probes: {:.2?}{}",
        chunk_idx,
        vmaf_cq_scores,
