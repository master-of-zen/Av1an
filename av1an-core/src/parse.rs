...
pub unsafe fn parse_aom_frames_sse41(s: &[u8]) -> Option<u64> {
    ...
    //
    // Pass 1/1 frame 102262/102261 136473850B  131502 ms 777.65 fps [ETA
    // unknown]    1272F                       ^^^^^^
    ...
    // Since the aomenc output follows a particular pattern, we can calculate
    // the position of the '/' character from the index of the first space
    // (how to do so is explained later on). We create this mask to find the
    ...
    // ignored prefix. This is safe because we already asserted that at
    ...
    //                    ^^^
    //                    These bits correspond to the first 3 characters:
    // "141".                    Since they do not match the space, they are
    // set to 0 in the mask.                    As printed, the leftmost bit
    ...
    let dynamic_mask =
        _mm_cmplt_epi8(ascending, _mm_set1_epi8(num_digits as i8));
    ...
    for s in help_text.split_ascii_whitespace() {
        if s.starts_with('-') {
            ...
            if encoder == Encoder::x265 {
                ...
                // So we need to ensure that in this case the short parameter is
                // also handled.
                ...
                // Somehow x265 manages to have a buggy --help, where a single
                // option (--[no-]-hrd-concat) has an extra
                // dash.
            ...
            // aomenc outputs '--tune=<arg>' for example, so we have to find the
            // character from the left so as to not miss the
            // leftmost char
            if let Some(idx) = s.find(|c: char| {
                !c.is_ascii_alphanumeric() && c != '-' && c != '_'
            }) {
                // In some weird cases (like with x264) there may be a dash
                // followed by a non alphanumeric character, so
                // we just ignore that.
                if idx > 1 {
...
mod tests {
    ...
    fn rav1e_parsing() {
        let test_cases = [
            ...
            (
                "encoded 12 frames, 126.416 fps, 16.32 Kb/s, elap. time: 1m \
                 36s",
                Some(12),
            ),
            (
                "encoded 122 frames, 126.416 fps, 16.32 Kb/s, elap. time: 1m \
                 36s",
                Some(122),
            ),
            (
                "encoded 1220 frames, 126.416 fps, 16.32 Kb/s, elap. time: 1m \
                 36s",
                Some(1220),
            ),
            (
                "encoded 12207 frames, 126.416 fps, 16.32 Kb/s, elap. time: \
                 1m 36s",
                Some(12207),
            ),
            ("invalid", None),
            (
                "encoded xxxx frames, 126.416 fps, 16.32 Kb/s, elap. time: 1m \
                 36s",
                None,
            ),
        ];
    ...
    fn svt_av1_parsing() {
        let test_cases = [
            ...
            ("Encoding frame 53298734 1.08 kbps 2.00 fps", Some(53_298_734)),
            ("invalid input", None),
            ("", None),
        ];
    ...
    fn AOM_parsing() {
        ...
        let test_cases =
            [
                (
                    "Pass 1/1 frame    3/2       2131B    5997 us 500.25 fps \
                     [ETA  unknown]",
                    Some(2),
                ),
                (
                    "Pass 2/2 frame   84/83     81091B  132314 us 634.85 fps \
                     [ETA  unknown]",
                    Some(83),
                ),
                (
                    "Pass 1/1 frame  142/141   156465B  208875 us 679.83 fps \
                     [ETA  unknown]",
                    Some(141),
                ),
                (
                ...
                (
                    "Pass 2/2 frame 4232/4231 5622510B 5518075 us 766.93 fps \
                     [ETA  unknown]",
                    Some(4231),
                ),
                (
                    "Pass 1/1 frame 13380/13379 17860525B   16760 ms 798.31 \
                     fps [ETA  unknown]",
                    Some(13379),
                ),
                (
                    "Pass 1/2 frame 13380/13379 17860525B   16760 ms 798.31 \
                     fps [ETA  unknown]",
                    Some(13379),
                ),
                ("invalid data", None),
                (
                    "Pass 2/2 frame 102262/102261 136473850B  131502 ms \
                     777.65 fps [ETA  unknown]    1272F",
                    Some(102_261),
                ),
                (
                    "Pass 1/1 frame 1022621/1022611 136473850B  131502 ms \
                     777.65 fps [ETA  unknown]    1272F",
                    Some(1_022_611),
                ),
                (
                    "Pass 1/1 frame 10000/9999 5319227B 8663074 us 1154.32 \
                     fps [ETA  unknown]",
                    Some(9999),
                ),
            ];
