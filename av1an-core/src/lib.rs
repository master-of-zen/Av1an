...
pub enum Input {
    VapourSynth { path: PathBuf, vspipe_args: Vec<String> },
    Video { path: PathBuf },
}
...
impl Input {
    ...
    pub fn as_video_path(&self) -> &Path {
        match &self {
            ...
            Input::VapourSynth {
                ...
                panic!(
                    "called `Input::as_video_path()` on an \
                     `Input::VapourSynth` variant"
                )
            },
        }
    }
    ...
    pub fn as_vapoursynth_path(&self) -> &Path {
        match &self {
            ...
            Input::Video {
                ...
                panic!(
                    "called `Input::as_vapoursynth_path()` on an \
                     `Input::Video` variant"
                )
            },
        }
    ...
    pub fn pixel_format(&self) -> anyhow::Result<String> {
        ...
        Ok(match self {
            Input::VapourSynth {
                path, ..
            } => crate::vapoursynth::pixel_format(
                path,
                self.as_vspipe_args_map()?,
            )
            .map_err(|_| anyhow::anyhow!(FAIL_MSG))?,
    ...
    fn transfer_function(&self) -> anyhow::Result<TransferFunction> {
        const FAIL_MSG: &str =
            "Failed to get transfer characteristics for input video";
        Ok(match self {
            Input::VapourSynth {
                path, ..
            } => {
                match crate::vapoursynth::transfer_characteristics(
                    path,
                    self.as_vspipe_args_map()?,
                )
                .map_err(|_| anyhow::anyhow!(FAIL_MSG))?
    ...
    pub fn transfer_function_params_adjusted(
        ...
        if enc_params.iter().any(|p| {
            let p = p.to_ascii_lowercase();
            // If the user specified an SDR transfer characteristic, assume they
            // want to encode to SDR.
            p.ends_with("bt709")
                || p.ends_with("bt.709")
                || p.ends_with("bt601")
                || p.ends_with("bt.601")
    ...
    /// will be empty.
    pub fn as_vspipe_args_map(
        &self,
    ) -> Result<OwnedMap<'static>, anyhow::Error> {
        ...
        for arg in self.as_vspipe_args_vec()? {
            ...
            if args_map
                .set_data(split[0], split[1].as_bytes())
                .is_err()
            {
...
#[derive(
    Serialize, Deserialize, Debug, EnumString, IntoStaticStr, Display, Clone,
)]
...
pub fn determine_workers(encoder: Encoder) -> u64 {
    ...
    std::cmp::max(
        match encoder {
            Encoder::aom | Encoder::rav1e => std::cmp::min(
                (cpu as f64 / 3.0).round() as u64,
                (ram_gb as f64 / 1.5).round() as u64,
            ),
            Encoder::svt_av1 | Encoder::x264 | Encoder::x265 => {
