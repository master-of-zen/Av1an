...
pub fn segment(input: impl AsRef<Path>, cache: impl AsRef<Path>, segments: &[usize]) {
  ...
  if segments.is_empty() {
    let split_path = Path::new(cache).join("split").join("0.mkv");
    ...
    let split_path = Path::new(cache).join("split").join("%05d.mkv");
    cmd.arg(split_path);
  }
