...
AOM_KEYFRAMES_DEFAULT_PARAMS = [
    '--threads=64', '--cpu-used=6', '--end-usage=q', '--tile-columns=2',
    '--tile-rows=1', '--cq-level=40'
]
...
def test_candidate_kf(dict_list, current_frame_index, frame_count_so_far):
    ...
    if ((qmode == False) or (frame_count_so_far > 2)) and (
    ...
    ) and (f['pcnt_second_ref'] < second_ref_usage_thresh) and (
        ...
        for i in range(0, 16):
            ...
            if (lnf['pcnt_inter'] < 0.05) or (next_iiratio < 1.5) or (
                ((lnf['pcnt_inter'] - lnf['pcnt_neutral']) < 0.20) and
                (next_iiratio < 3.0)) or ((boost_score - old_boost_score) <
                                          3.0) or (lnf['intra_error'] < 200):
                break
