...
def save_chunk_queue(temp: Path, chunk_queue: List[Chunk]) -> None:
    ...
    with open(Path(temp) / "chunks.json", "w") as file:
...
def create_video_queue_hybrid(
    ...
    keyframes = get_keyframes(str(project.input))
    ...
    segment(str(project.input), str(project.temp), to_split[1:])
    log("Segment Done")
    source_path = (Path(project.temp) / "split").as_posix()
    queue_files = [x for x in Path(source_path).iterdir() if x.suffix == ".mkv"]
...
def create_video_queue_vs(project: Project, split_locations: List[int]) -> List[Chunk]:
    ...
    source_file = Path(project.input).absolute().as_posix()
    if project.is_vs:
        vs_script = project.input
    else:
        vs_script = create_vs_file(
            Path(project.temp).as_posix(), source_file, project.chunk_method
        )
...
def create_select_chunk(
    project: Project, index: int, src_path: str, frame_start: int, frame_end: int
) -> Chunk:
    assert frame_end > frame_start, "Can't make a chunk with <= 0 frames!"
